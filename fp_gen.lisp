		...
		(mapcar
		  (lambda (b2zb)
			(let*
			  ...
			  (printf '\t%s[%d]%t?%t%s==0%t?%t%s[%d:%d]%t:%t%s[%d:%d]%t:%t\n c2z c2zb ez f2z (+ 1 f2z_msb) (+ 1 f2z_lsb) f2z f2z_msb f2z_lsb)))
  ...
  (lambda (a b z sign_bit expt_msb expt_lsb frac_msb frac_lsb)
	(let*
	  ...
	  (progn
		...
		(printf 'wire%t[%d:0]%t%s%t=%t%s==0%t?%t%s[%d:%d]%t:%t%s%t?%t%s[%d:%d]%t:%t%s[%d:%d];\n frac_msb fz ez f1z (+ (* 2 (+ frac_sb 1)) -3) (- (+ (* 2 (+ frac_sb 1)) -3) (- frac_sb 1)) cf1z f1z (+ (* 2 (+ frac_sb 1)) -2) (- (+ (* 2 (+ frac_sb 1)) -2) (- frac_sb 1)) f1z (+ (* 2 (+ frac_sb 1)) -3) (- (+ (* 2 (+ frac_sb 1)) -3) (- frac_sb 1)))
		(printf 'assign%t%s%t=%t{%s,%s,%s};\n z sz ez fz)
