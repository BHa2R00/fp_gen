...
(define verilog-fp_add
  (lambda (a b z sign_bit expt_msb expt_lsb frac_msb frac_lsb)
	(let*
	  ...
	  (seb (s+ 'sign_expt_ b))
	  (meb (s+ 'mant_expt_ b))
	  (na (s+ 'not_a_number_ z))
	  (dnm (s+ 'denormalized_ z))
	  ...
	  (progn
		...
		(printf 'wire%t%a%t=%t%a;\n na (list (s+ ea '== (itoa '%d expt_sb) (itoa '%c 39) (itob (+ -1 (expt 2 expt_sb)) (+ -1 expt_sb) 0)) '| (s+ eb '== (itoa '%d expt_sb) (itoa '%c 39) (itob (+ -1 (expt 2 expt_sb)) (+ -1 expt_sb) 0))))
		(printf 'wire%t%a%t=%t%a;\n dnm (list (s+ ea '== (itoa '%d expt_sb) (itoa '%c 39) (itob 0 (+ -1 expt_sb) 0)) '| (s+ eb '== (itoa '%d expt_sb) (itoa '%c 39) (itob 0 (+ -1 expt_sb) 0))))
		(printf 'wire%t%s%t=%t\n sdeab)
		...
		(printf '\t0;\n)
		(printf 'wire%t[1:0]%t%s%t=%t%s==0%t?%t2\'d0%t:%t2\'d1;\n bfa ea)
		(printf 'wire%t[1:0]%t%s%t=%t%s==0%t?%t2\'d0%t:%t2\'d1;\n bfb eb)
		(printf 'wire%t[%d:0]%t%s%t=%t{%s,%s[%d:0],%d\'d0};\n (+ 2 frac_sb expt_mask -1) fa bfa a frac_msb expt_mask)
		(printf 'wire%t[%d:0]%t%s%t=%t{%s,%s[%d:0],%d\'d0};\n (+ 2 frac_sb expt_mask -1) fb bfb b frac_msb expt_mask)
		...
		(printf '\t0;\n)
		(printf 'wire%t[%d:0]%t%s%t=%t\n (- expt_sb 1) e3)
		(printf '\t%a%t?%t%a%t:%t\n na (s+ (itoa '%d expt_sb) (itoa '%c 39) (itob (+ -1 (expt 2 expt_sb)) (+ -1 expt_sb) 0)))
		(printf '\t%a%t?%t%a%t:%t\n dnm (s+ (itoa '%d expt_sb) (itoa '%cd0 39)))
		(printf '\t%s%t?%t\n sdeab)
		(printf '\t%l%s%t?%t%s%t-%t%s%t:%t%s%t+%t%s%r%t:%t\n sie2 eb mie2 eb mie2)
		(printf '\t%l%s%t?%t%s%t-%t%s%t:%t%s%t+%t%s%r;%t\n sie2 ea mie2 ea mie2)
		(printf 'wire%t[%d:0]%t%s%t=%t\n frac_msb f3)
		(printf '\t%a%t?%t%a%t:%t\n na (s+ (itoa '%d frac_sb) (itoa '%cd0 39)))
		(mapcar
		  (lambda (b2zb)
			(let*
			  (c2zb (- (+ frac_sb 2 -1) b2zb))
			  ...
			  (printf '\t%s[%d]%t?%t%s==0%t?%t%s[%d:%d]%t:%t%s[%d:%d]%t:%t\n c2z c2zb e3 f2z (+ 1 f2z_msb) (+ 1 f2z_lsb) f2z f2z_msb f2z_lsb)))
		  (linspace 0 1 (+ frac_sb 2 -1)))
		(printf '\t0;\n)
		(printf 'wire%t%a%t=%t%a;\n of (list e3 '> (s+ (itoa '%d expt_sb) (itoa '%cd 39) (itoa '%d (+ -2 (expt 2 expt_sb))))))
		(printf 'wire%t%a%t=%t%a;\n uf (list e3 '< (s+ (itoa '%d expt_sb) (itoa '%cd 39) (itoa '%d 1))))
		(printf 'wire%t%s%t=%t%s;\n sz s2z)
		(printf 'wire%t[%d:0]%t%a%t=%t%a;\n (+ -1 expt_sb) ez (list of '? (s+ (itoa '%d expt_sb) (itoa '%cd 39) (itoa '%d (+ -1 (expt 2 expt_sb)))) ': (list uf '? (s+ (itoa '%d expt_sb) (itoa '%cd 39) (itoa '%d 0)) ': e3)))
		(printf 'wire%t[%d:0]%t%a%t=%t%a;\n (+ -1 frac_sb) fz (list of '? (s+ (itoa '%d frac_sb) (itoa '%cd 39) (itoa '%d 0)) ': (list uf '? (s+ (itoa '%d frac_sb) (itoa '%cd 39) (itoa '%d 0)) ': f3)))
  ...
  (lambda (a b z sign_bit expt_msb expt_lsb frac_msb frac_lsb)
	(let*
	  ...
	  (progn
		...
		(printf 'wire%t[%d:0]%t%s%t=%t%s[%d:%d];\n (- expt_sb 1) ea a expt_msb expt_lsb)
		(printf 'wire%t[%d:0]%t%s%t=%t%s[%d:%d];\n (- expt_sb 1) eb b expt_msb expt_lsb)
		(printf 'wire%t%s%t=%t%s==0%t?%t0%t:%t1%t;\n bfa ea)
		(printf 'wire%t%s%t=%t%s==0%t?%t0%t:%t1%t;\n bfb eb)
		(printf 'wire%t[%d:0]%t%s%t=%t{%s,%s[%d:0]};\n frac_sb fa bfa a frac_msb)
		(printf 'wire%t[%d:0]%t%s%t=%t{%s,%s[%d:0]};\n frac_sb fb bfb b frac_msb)
